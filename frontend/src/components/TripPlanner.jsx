...
import { X, Hotel, UtensilsCrossed, Umbrella, Users, ChevronRight, ChevronLeft, CheckCircle, Clock, Loader2 } from 'lucide-react';
...
function VenueSelect({ label, value, onChange, options, testId }) {
  return (
    <select value={value} onChange={e => onChange(e.target.value)} className="w-full px-3 py-2.5 text-sm border border-stone-300 rounded-lg bg-stone-50 text-stone-700 focus:outline-none focus:ring-2 focus:ring-stone-400/30" data-testid={testId}>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" data-testid="trip-planner-modal">
      ...
      <div className="relative w-full max-w-lg mx-4 bg-[#F5F2EB] rounded-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div className="bg-stone-800 px-6 py-5 flex items-center justify-between flex-shrink-0">
          <div>
            <h2 className="text-white font-heading text-xl">Plan a Trip or Make a Reservation</h2>
            <p className="text-stone-400 text-xs mt-0.5">Step {step} of 3</p>
          </div>
        ...
        <div className="h-1 bg-stone-300 flex-shrink-0">
          <div className="h-full bg-stone-700 transition-all duration-500 ease-out" style={{ width: `${(step / 3) * 100}%` }} />
        </div>
        ...
        {!submitted && (
          <div className="px-6 py-4 border-t border-stone-300 flex items-center justify-between flex-shrink-0 bg-[#F5F2EB]">
            ...
            {step < 3 ? (
              <button onClick={() => setStep(s => s + 1)} disabled={!canNext()} className="flex items-center gap-1.5 px-5 py-2.5 bg-stone-700 text-white text-sm rounded-lg hover:bg-stone-800 transition-colors disabled:opacity-40 disabled:cursor-not-allowed" data-testid="trip-next-btn">
    ...
    <div data-testid="trip-planner-step-2">
      ...
      <div className="flex justify-center mb-5">
        <Calendar
          ...
          className="rounded-xl border border-stone-300 shadow-sm bg-white"
          classNames={calendarClassNames}
        />
      </div>
      {form.date && (
        <p className="text-center text-sm text-stone-600 mb-4">
          Selected: <span className="font-semibold text-stone-800">{formatDate(form.date)}</span>
        </p>
      )}
      <div className="mb-5">
...
function StepThree({ form, setForm, formatDate }) {
  ...
  return (
    <div data-testid="trip-planner-step-3">
      ...
      <div className="mt-5 p-4 bg-stone-100 rounded-xl border border-stone-200">
