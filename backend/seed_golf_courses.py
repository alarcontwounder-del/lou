...
async def seed_golf_courses():
    ...
    print("🏌️ Starting golf courses migration...")
    
    # Check if collection already has data
    existing_count = await db.golf_courses.count_documents({})
    if existing_count > 0:
        print(f"⚠️  Collection already has {existing_count} courses.")
    ...
    print(f"✅ Inserted {len(result.inserted_ids)} golf courses")
    ...
    print("📇 Created indexes on 'id' and 'display_order'")
    
    # Verify
    final_count = await db.golf_courses.count_documents({})
    print(f"🎉 Migration complete! Total courses in database: {final_count}")
...
async def reset_golf_courses():
    ...
    print("🔄 Resetting golf courses to seed data...")
    ...
    result = await db.golf_courses.insert_many(courses_with_timestamps)
    print(f"✅ Reset complete! {len(result.inserted_ids)} courses restored.")
